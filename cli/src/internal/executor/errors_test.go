...
func TestConfigValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "Valid config - empty shell",
			config:  Config{Shell: ""},
			wantErr: false,
		},
		{
			name:    "Valid config - bash",
			config:  Config{Shell: "bash"},
			wantErr: false,
		},
		{
			name:    "Valid config - pwsh",
			config:  Config{Shell: "pwsh"},
			wantErr: false,
		},
		{
			name:    "Invalid shell",
			config:  Config{Shell: "invalid"},
			wantErr: true,
		},
		{
			name:    "Valid shell with uppercase",
			config:  Config{Shell: "BASH"},
			wantErr: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr && err != nil {
				if _, ok := err.(*InvalidShellError); !ok {
