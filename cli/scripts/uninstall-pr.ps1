...
<#
.SYNOPSIS
    Uninstall a PR build of the azd exec extension
.DESCRIPTION
...
.EXAMPLE
    iex "& { $(irm https://raw.githubusercontent.com/jongio/azd-exec/main/cli/scripts/uninstall-pr.ps1) } -PrNumber 123"
#>
...
Write-Host "🗑️  Uninstalling azd exec PR build" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path "pr-registry.json") {
    ...
    Remove-Item -Force "pr-registry.json"
    Write-Host "   ✓" -ForegroundColor DarkGray
}
